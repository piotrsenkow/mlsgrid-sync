...
- [ ] **M1 — Scaffold.** GitHub repo (public, Apache-2.0 + NOTICE), CI (lint / test -race / build matrix), Makefile, Dockerfile, cobra CLI skeleton (`init-db`, `backfill`, `sync`, `reconcile`, `media`, `status`, `version`), viper config loader with multi-MLS profiles, `.env.example`, CLAUDE.md, README stub with compliance section.
